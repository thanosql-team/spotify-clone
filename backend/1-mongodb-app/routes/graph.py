...
async def deep_graph_recommendations(
    ...
    Finds similar users using recursive LISTENED_TO relationships
    and recommends songs they listened to.
    """
    neo4j = await get_neo4j()
    ...
    // 1. Direct similarity
    MATCH (u)-[:LISTENED_TO]->(:Song)<-[:LISTENED_TO]-(other:User)
    WITH u, other, count(*) AS directScore

    // 2. Deep similarity (recursive)
    OPTIONAL MATCH path = (u)-[:LISTENED_TO*2..4]-(other)

    WITH u, other,
        directScore +
        CASE WHEN path IS NULL THEN 0 ELSE size(nodes(path)) END
        AS similarity
    WHERE other.user_id <> $user_id AND similarity > 0

    // 3. Recommend songs from similar users
    MATCH (other)-[:LISTENED_TO]->(song:Song)
    WHERE NOT (u)-[:LISTENED_TO]->(song)

    RETURN DISTINCT song.song_id AS song_id,
        song.name AS song_name,
        max(similarity) AS relevance
    ORDER BY relevance DESC, song_name ASC
    LIMIT $limit
    """
